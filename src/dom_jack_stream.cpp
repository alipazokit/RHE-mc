...
//      cout<<step_size<<endl;
  ...
  if(dominance==true){
        for (int i=0;i<real_Njack;i++){
                double sum=0;
                for(int j=0;j<Nbin;j++){
                        sum+=jack_bin_real[i][j];

                }
                jack_bin_real[i][Nbin]=sum;
        }
...
//	cout<<step_size<<endl;
	...
	jack_bin.resize(Njack, vector<int>(Nbin,0));
	 ...
	 jack_bin.resize(Njack, vector<int>(Nbin+1,0));
	  jack_bin_real.resize(real_Njack, vector<int>(Nbin+1,0));
...
cout<<"bin"<<j<<" "<<temp<<endl;
}


cout<<"jackbin_real"<<endl;
        for (int i=0;i<real_Njack;i++){
           for(int j=0;j<Nbin;j++)
                cout<<jack_bin_real[i][j]<<" ";
          cout<<endl;
        }
...
cout<<"finished reading annotation"<<endl;
}
  ...
  //      cout<<"eee"<<endl;
	 ...
	 output=compute_XXz(num_snp);
	   ...
	   for (int z_index=0;z_index<Nz;z_index++){
		...
		if(dominance==false){
			...
			// if(num_snp!=len[bin_index])
                           XXz.col((Nbin*(real_Njack+1)*Nz)+(jack_index_real*Nz)+z_index)+=output.col(z_index);
                        XXz.col((Nbin*(real_Njack+1)*Nz)+(real_Njack*Nz)+z_index)+=output.col(z_index);  
		 ...
		 if(both_side_cov==true) {
		  ...
		  if(dominance==false){
		  	...
		  	UXXz.col((bin_index*(real_Njack+1)*Nz)+(real_Njack*Nz)+z_index)+=w3;
				 ...
				 UXXz.col((Nbin*(real_Njack+1)*Nz)+(jack_index_real*Nz)+z_index)+=w3;
                                 UXXz.col((Nbin*(real_Njack+1)*Nz)+(real_Njack*Nz)+z_index)+=w3;
	   ...
	   if (both_side_cov==true){
	      ...
	      for (int z_index=0;z_index<Nz;z_index++){
		   if(dominance==false){
			 ...
			 XXUz.col((Nbin*(real_Njack+1)*Nz)+(jack_index_real*Nz)+z_index)+=output.col(z_index);
			  XXUz.col((Nbin*(real_Njack+1)*Nz)+(real_Njack*Nz)+z_index)+=output.col(z_index);
	...
	double temp_val;
	   if(both_side_cov==false){
		  temp_val=compute_yXXy(num_snp);
	      if(dominance==false)
		yXXy(bin_index,jack_index_real)+=temp_val;
	      else{
		yXXy(Nbin,jack_index_real)+=temp_val;
	      }
		
	   }	
	   else{
		temp_val=compute_yVXXVy(num_snp);
	       if(dominance==false)		
	       	  yXXy(bin_index,jack_index_real)=+temp_val;
	       else{
		 yXXy(Nbin,jack_index_real)+=temp_val;
	   ...
	   if(dominance==false)
           	yXXy(bin_index,real_Njack)+=temp_val;
	   else
		yXXy(Nbin,real_Njack)+=temp_val;	
...
///////////////////////////////////////////
	...
	Njack=Njack*10;
	 ...
	 //real_Njack=Njack/10;
        cout<<"number of jackknife block "<<real_Njack<<endl;
	cout<<"number of streaming block "<<Njack<<endl;
...
XXz=MatrixXdr::Zero(Nindv,total_Nbin*(real_Njack+1)*Nz);

if(both_side_cov==true){
UXXz=MatrixXdr::Zero(Nindv,total_Nbin*(real_Njack+1)*Nz);
XXUz=MatrixXdr::Zero(Nindv,total_Nbin*(real_Njack+1)*Nz);
//Xz=MatrixXdr::Zero(Nindv,Nbin*(Njack+1)*Nz);
}
yXXy=MatrixXdr::Zero(total_Nbin,real_Njack+1);
...
for (jack_index=0;jack_index<=Njack;jack_index++){
  for (int i=0;i<Nbin;i++){
	...
	if( jack_index<Njack && len[i]==jack_bin_real[jack_index][i])
...
for (int i=0;i<Njack;i++){
    ...
    total_size=0;
    for (int j=0;j<Nbin;j++){
        total_size+=(len[j]-jack_bin_real[i][j]);
    }
   for (int j=0;j<Nbin;j++){
